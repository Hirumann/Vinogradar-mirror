...
$('#gantt-view').on('click', function() {
    ...
    $('.view-button').removeClass('bg-gray-500 bg-blue-500').addClass('bg-gray-500');
...
$(document).on('click', '.set-range', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            loadMiniCalendar(currentDateMiniCalendar.getFullYear(), currentDateMiniCalendar.getMonth(), startDate, endDate);
        }
    });
});
...
$('#mini-calendar').on('click', '.mini-calendar-day', function() {
    ...
    switch (numberOfClicks) {
        case 0:
            ...
            $('#end-date').val(null);
            $('.mini-calendar-day').removeClass('bg-red-300 bg-blue-500 bg-green-400');
...
function checkDate(date, startDate, endDate) {
    ...
    if (start && end) {
        if (currentDateNow >= start || currentDateNow <= end) {
            if (currentDateNow.toLocaleDateString() === start.toLocaleDateString()) {
                console.log('= start');
                return 'bg-green-400';
            } else if (currentDateNow.toLocaleDateString() === end.toLocaleDateString()) {
                console.log('= end');
                return 'bg-blue-500';
...
function buildGanttTable(data, view) {
    ...
    for (let i = 0; i < columnCount; i++) {
        ...
        $dynamicColumns.append(`<th class="border px-2 py-2">${getColumnLabel(columnDate, view)}</th>`);
    }
    ...
    data.forEach((item, index) => {
        ...
        const duration = Math.round((endItemDate - startItemDate) / (1000 * 60 * 60 * 24)) + 1;
        ...
        for (let i = 0; i < columnCount; i++) {
            ...
            if (view === 'day') {
                ...
                const maxWeekEndDate = Math.min(columnStartDate.getDate() + 6, daysInMonth);
            ...
            if (shouldPaintCell(startItemDate, endItemDate, columnStartDate, columnEndDate)) {
                applyPartialCellPaint($cell, startItemDate, endItemDate, columnStartDate, columnEndDate);
            }
...
function getColumnLabel(date, view) {
    ...
    if (view === 'day') {
        label = formatDate(date).split('.')[0] + '.' + formatDate(date).split('.')[1];
    } else if (view === 'week') {
        label = `${formatDate(date).split('.')[1] + '.' + formatDate(date).split('.')[2]}<br>${Math.ceil((date.getDate() / 7))} нед.`;
    } else if (view === 'month') {
...
function shouldPaintCell(startDate, endDate, columnStartDate, columnEndDate) {
    ...
    const overlaps = (startDateOnly < columnEndDate) && (endDateOnly > columnStartDate);
...
function applyPartialCellPaint($cell, startDate, endDate, columnStartDate, columnEndDate) {
    if (columnStartDate.getDate() === columnEndDate.getDate()) {
        startDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
        endDate = new Date(endDate.getFullYear(), endDate.getMonth(), endDate.getDate(), 23, 59, 59, 999);
        columnStartDate = new Date(columnStartDate.getFullYear(), columnStartDate.getMonth(), columnStartDate.getDate());
        columnEndDate = new Date(columnEndDate.getFullYear(), columnEndDate.getMonth(), columnEndDate.getDate(), 23, 59, 59, 999);
    }
    ...
    const totalColumnDuration = (columnEndDate - columnStartDate) / (1000 * 60 * 60 * 24) + 1; // Включаем последний день
    const eventStartInColumn = (cellStart - columnStartDate) / (1000 * 60 * 60 * 24);
    const eventEndInColumn = (cellEnd - columnStartDate) / (1000 * 60 * 60 * 24);

    // Ограничиваем значения
    const startPercentage = Math.max(0, eventStartInColumn / totalColumnDuration * 100);
    const endPercentage = Math.min(100, (eventEndInColumn + 1) / totalColumnDuration * 100);
